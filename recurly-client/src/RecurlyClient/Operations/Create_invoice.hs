...
module RecurlyClient.Operations.Create_invoice where
...
create_invoice ::
    ...
    RecurlyClient.Common.ClientT m (Network.HTTP.Client.Types.Response Create_invoiceResponse)
create_invoice
    account_id
    body =
        GHC.Base.fmap
            ( \response_0 ->
                GHC.Base.fmap
                    ( Data.Either.either Create_invoiceResponseError GHC.Base.id
...
data Create_invoiceResponse
    = -- | Means either no matching case available or a parse error
      Create_invoiceResponseError GHC.Base.String
...
-}
create_invoiceWithConfiguration ::
    forall m.
    (RecurlyClient.Common.MonadHTTP m) =>
    ...
    InvoiceCreate ->
    -- | Monadic computation which returns the result of the operation
    m (Network.HTTP.Client.Types.Response Create_invoiceResponse)
create_invoiceWithConfiguration
    config
    account_id
    body =
...
The same as 'create_invoice' but returns the raw 'Data.ByteString.ByteString'.
-}
create_invoiceRaw ::
    forall m.
    (RecurlyClient.Common.MonadHTTP m) =>
...
The same as 'create_invoice' but accepts an explicit configuration and returns the raw 'Data.ByteString.ByteString'.
-}
create_invoiceWithConfigurationRaw ::
    forall m.
    (RecurlyClient.Common.MonadHTTP m) =>
    -- | The configuration to use in the request
    RecurlyClient.Common.Configuration ->
