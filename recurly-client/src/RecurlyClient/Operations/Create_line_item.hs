...
module RecurlyClient.Operations.Create_line_item where
...
create_line_item ::
    ...
    RecurlyClient.Common.ClientT m (Network.HTTP.Client.Types.Response Create_line_itemResponse)
create_line_item
    account_id
    body =
        GHC.Base.fmap
            ( \response_0 ->
                GHC.Base.fmap
                    ( Data.Either.either Create_line_itemResponseError GHC.Base.id
                        GHC.Base.. ( \response body ->
                                        if
                                            ...
                                            | GHC.Base.const GHC.Types.True (Network.HTTP.Client.Types.responseStatus response) ->
                                                Create_line_itemResponseDefault
                                                    Data.Functor.<$> ( Data.Aeson.eitherDecodeStrict body ::
                                                                        Data.Either.Either
                                                                            GHC.Base.String
                                                                            Error
...
data Create_line_itemResponse
    = -- | Means either no matching case available or a parse error
      Create_line_itemResponseError GHC.Base.String
...
-}
create_line_itemWithConfiguration ::
    forall m.
    (RecurlyClient.Common.MonadHTTP m) =>
    ...
    LineItemCreate ->
    -- | Monadic computation which returns the result of the operation
    m (Network.HTTP.Client.Types.Response Create_line_itemResponse)
create_line_itemWithConfiguration
    config
    account_id
    body =
...
The same as 'create_line_item' but returns the raw 'Data.ByteString.ByteString'.
-}
create_line_itemRaw ::
    forall m.
    (RecurlyClient.Common.MonadHTTP m) =>
...
The same as 'create_line_item' but accepts an explicit configuration and returns the raw 'Data.ByteString.ByteString'.
-}
create_line_itemWithConfigurationRaw ::
    forall m.
    (RecurlyClient.Common.MonadHTTP m) =>
    -- | The configuration to use in the request
    RecurlyClient.Common.Configuration ->
